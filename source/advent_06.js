...
function drawImage(seed) {
    ...
    const border = 250  // pixels comprising the border
    ...
    for (let i = 0; i < bubbles; i++) {
        ...
        if(cell.row > 0 & cell.row < (grain - aura)) {
            if(cell.row > 0 & cell.row < (grain - aura)) {
                ...
                // determine whether this counts as a valid merge 
                let merge = neighbours.map(c => c.valid).reduce((a,b) => a & b)
                
                // set the whole neighbourhood to be invalid targets, regardless of 
                // whether a merge takes place
    ...
    function drawCircle(grid, radii, palette) {
        ...
        if(cell.valid) {
            ...
            //ctx.rect(cell.x - radius, cell.y - radius, radius * 2, radius * 2);
            ctx.stroke();
            if (rng.double() < .5) {
                ctx.fill()
            }
    ...
    function drawBarbell(grid, radii, palette) {
        ...
        if(cell1.valid) {
            ...
            if(cell2.valid) {
                ...
                // set linewidth
                ctx.lineWidth = 2 + Math.min(radius1, radius2) / 3

                // randomly define the arc
                var arcMid
                if (vertical) {
                    if (cell2.y < cell1.y) {
                        arcMid = -Math.PI / 2
                ...
                var arc1 = arcMid + Math.PI * (.1 + .9 * rng.double() / 2)
                var arc2 = arcMid - Math.PI * (.1 + .9 * rng.double() / 2)

                // draw filled rectangle in the primary location
                ctx.beginPath();
                ctx.fillStyle = shade;
                ctx.strokeStyle = shade;
                ctx.arc(cell1.x, cell1.y, radius1, 0, Math.PI * 2, true);
                //ctx.rect(cell1.x - radius1, cell1.y - radius1, radius1 * 2, radius1 * 2);
                ctx.stroke();
                ctx.fill();
                ctx.closePath();
                ...
                ctx.fillStyle = shade
                ctx.strokeStyle = shade
                ctx.arc(cell2.x, cell2.y, radius2, arc1, arc2, false)
                ctx.stroke();
                ctx.closePath();
    ...
    // path to file
    let file = "../output/06/advent_06_" + seed + ".png"
