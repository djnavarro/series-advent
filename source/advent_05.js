...
function drawImage(seed) {
    ...
    function drawBarbell(grid, radii, palette) {
        ...
        if(cell1.valid) {
            ...
            if (vertical) {
                slice = grid.filter(c => (c.col == cell1.col) & (Math.abs(c.row - cell1.row) < 3))
            } else {
                slice = grid.filter(c => (c.row == cell1.row) & (Math.abs(c.col - cell1.col) < 3))                
            }
            ...
            if(cell2.valid) {
                ...
                // linewidth for all barbell segments
                ctx.lineWidth = Math.min(radius1, radius2) / 2

                // draw circle in the primary location
                ctx.beginPath();
                ctx.fillStyle = shade;
                ctx.strokeStyle = shade;
                ...
                // draw arc in the secondary location
                ctx.beginPath();
                ctx.fillStyle = shade
                ctx.strokeStyle = shade
    ...
    // path to file
    let file = "../output/05/advent_05_" + seed + ".png"
